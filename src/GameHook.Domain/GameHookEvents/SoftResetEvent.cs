...
namespace GameHook.Domain.GameHookEvents
{
    public class SoftResetEvent : GameHookEvent, IGameHookEvent
    {
        ...
        {
            SetEvent("_softreset", 0, 0, null, 1, 0);
        }
        ...
        public override void SetEvent(string? name, MemoryAddress address, ushort bank, string? bits, int length, int size)
        {
            if (Instance != null && Instance.Driver != null)
            {
                Instance.Driver.AddEvent("_softreset", 0, ushort.MaxValue, EventType.EventType_SoftReset, EventRegisterOverrides, bits, length, size);
            }
        }
    }
}
